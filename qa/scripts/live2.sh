...
# `./gradlew runClient` on every boot -- skipping that route's per-boot
# gradle config/task-graph-eval (and, on a cold daemon or after a source
# change, recompile + dev-env artifact reassembly) tax. Proven live in a
# throwaway sandbox (display :98, tmux hsqa-fastboot, port 25575 -- see
# qa/reports/ for the session that verified this): the fast client reaches
# a fully rendered main menu, with the current hearthstead jar loaded
# ("Hearthstead is kindling the fire..." in the log), in ~26s process-start
# to menu-ready, vs. this same day's live.sh session's own
# logs/live-client.log showing ModLauncher-up to joined-game at ~25s in a
# warm-Gradle-daemon/up-to-date-build run -- so the reliable win here is
# that this route's pre-JVM overhead is consistently near-zero and immune
# to Gradle's cold-daemon/recompile tax, not a large multiplier against a
# warm-daemon best case. See client_install.sh's header for exactly what it
# reuses from the Gradle dev workspace's caches and why.
#
# Swap-in: once verified for your case, point the coordinator at this file
# instead of live.sh -- same TMUX_SESSION/PORT/DISPLAY_NUM, same CLI
# surface (start/status/shot/key/.../stop), same evidence layout.
#
...
safe_regrab() {
    ...
    # Fourth fix (2026-08-26, found by the first SURVIVAL playthrough, ported
    # here from live.sh -- this twin had never received it and was carrying
    # the exact same lethal-in-survival bug KF-030 fixed there):
    ...
    if [ "${sky:-0}" -gt "$(cat "$STATE/sky_seen" 2>/dev/null || echo 0)" ]; then
        ...
        echo "live2.sh: safe_regrab skipped the grab click (survival, no clear sky overhead)" >&2
    fi
    srv_send "tp $player $x $y $z $yaw $pitch"
    sleep 1
}
...
# KF-035: verify-then-regrab, not regrab-and-hope. See KF-035 in
# docs/project/KNOWN_FAILURES.md for the full evidence trail (found and
# proven against live.sh; ported here unchanged since this twin shares the
# identical safe_regrab/GLFW-grab machinery). Round 1 found that late in a
# long session even a blind regrab click stopped helping -- not because the
# click itself had stopped working, but because nothing ever checked
# whether it had. This makes that check load-bearing and automatic.
#
...
# this is wired into `look`/`hold`/`mine`/`cmd` and deliberately NOT wired
...
start)
    ...
    rm -f "$STATE/player"
    # KF-035: $STATE/sky_seen is a monotonic "have I already seen this many
    # HSQA_SKY_CLEAR lines" counter that safe_regrab (survival branch)
    # writes but never resets. A value left over from a previous session can
    # outrun what a new session's own fresh log will ever reach, silently
    # disabling the survival-mode regrab click for the whole new session. A
    # brand new instance/log deserves a brand new baseline.
    rm -f "$STATE/sky_seen"
    ...
    # Fast path (D-H5): shared production-style client install instead of
    # `./gradlew runClient`. client_install.sh is itself idempotent (a
    ...
    NEOFORGE_ID=$(basename "$(ls -d "$CLIENT_INSTALL_DIR"/versions/neoforge-*/ 2>/dev/null | head -1)")
    CLIENT_MC_VERSION=$(basename "$(ls -d "$CLIENT_INSTALL_DIR"/versions/*/ 2>/dev/null | grep -v '/neoforge-' | head -1)")
    [ -n "$NEOFORGE_ID" ] && [ -n "$CLIENT_MC_VERSION" ] \
        || die client_install "could not resolve installed version dirs under $CLIENT_INSTALL_DIR/versions"
    mkdir -p "$RUN_DIR/mods"
    cp "$JAR" "$RUN_DIR/mods/"
    # --join uses --quickPlayMultiplayer, the same real vanilla game
    # argument (added upstream in 23w41a) that HSQA_JOIN drives through
    # build.gradle for the gradle-driven route above -- not a mod-specific
    # hook, so no dev-mode autoconnect equivalent had to be reimplemented.
    tmux new-window -d -t "$TMUX_SESSION" -n client \
        "cd '$RUN_DIR' && python3 '$CLIENT_INSTALL_DIR/launch.py' --install-dir '$CLIENT_INSTALL_DIR' --mc-version '$CLIENT_MC_VERSION' --neoforge-id '$NEOFORGE_ID' --game-dir '$RUN_DIR' --join '127.0.0.1:$PORT' 2>&1 | tee '$EV_LOGS/live-client.log'"
...
mine)   # KF-035: a first-class mining/attack primitive. See live.sh's own
        # comment for the full rationale; ported here unchanged.
        ensure_grab
        MINE_SECS="${2:-1.5}"
